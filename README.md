...
访问 `http://localhost:3000`。**规则拆解零 Key 必可演示**；数据默认在浏览器本地处理。

### 可选：增强拆解（双轨）

1. 复制 [`.env.example`](.env.example) 为 `.env`，填入 OpenAI 兼容 Key（勿提交仓库）。
2. 或在页面点「增强配置」，把 Base URL / Model / Key 写入 **sessionStorage**（关标签即失效）。
3. `GET /api/health`：`rules.demo_ready` 恒为 true；仅当服务端有 Key 时 `enhanced.demo_ready=true`。
4. 增强失败时 UI **大声回退规则拆解**，徽章显示「增强失败 · 已回退规则」。

创空间线上 Demo 默认为静态托管：无服务端 Key 时走规则路径；现场演示增强请用页面会话配置（注意浏览器 CORS 限制）。
...
1. 确认页眉模式徽章（无 Key 应为「规则拆解」）。
2. 点击「加载脱敏样例」→「拆解要求」。
3. 查看候选要求的原文引用和优先级，并确认或忽略候选。
4. 在证据矩阵中更新材料状态。
5. 勾选风险任务，查看受约束草稿及引用依据。
6. （可选）打开页脚「反馈」留下真实试用意见。

## 边界与限制

- 默认路径是可复现的**规则驱动**拆解；**可选** OpenAI 兼容增强，不作为线上零 Key 硬依赖。
- 候选确认、证据状态和任务完成状态仅保存在当前浏览器的 `localStorage` 中；它们不会同步给其他用户或服务器。
- 会话级 API Key 只存在 `sessionStorage`，不会写入创空间 Git 或本仓库。
- 请勿粘贴未脱敏的投标文件、客户资料、个人信息或商业机密。
- 草稿不是投标、法律、财务或合规意见；任何事实、报价、资格与承诺均须由负责人审核。
- 当前以纯文本候选要求为主，可能遗漏表格、附件、例外条款和非标准表述；生产使用前应补充分段解析、权限控制、审计和企业数据治理。
